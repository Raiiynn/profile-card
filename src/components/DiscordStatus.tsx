...
const DiscordStatus = ({ userId }: DiscordStatusProps) => {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        const response = await fetch(`https://api.lanyard.rest/v1/users/244313875351207936`);
        const result = await response.json();
        console.log('Lanyard API response:', result); // Debug log
        if (result.success) {
          setData(result.data);
        } else {
          console.error('Lanyard API error:', result.error); // Debug log
        }
      } catch (error) {
